...
| Primitive bodies built once and moved by model matrices: chassis/cabin, translucent glass, wheels that spin and steer, head/tail/brake/indicator/roof lamps, night beams, dashboard + steering wheel + wipers, paint darkening | `VehicleBody.ts` | screenshots `docs/screenshots/vehicle-driving-third.png` (rickshaw at Marine Drive, fixtures), `vehicle-driving-gateway.png` (e2e), `vehicle-dashboard.png` (bus, e2e), `vehicle-dashboard-rickshaw-rain.png` (wipers/rain), `vehicle-night-third.png` (headlights, roof light, beams) |
