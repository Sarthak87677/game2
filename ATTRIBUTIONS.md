...
## Procedural interiors and hero campus (track `interiors-campus`)

* **Interior grammar** (`src/world/interiors/grammar.ts`, vocabulary in `src/data/maharashtra/interiorGrammar.ts`) — every interior is generated by this project (MIT) from a building footprint, height and a generic room programme; nothing is a survey, scan or floor plan of a real building. The HUD shows "Generated interior — fictional, not surveyed" while one is active.
* **SGIS-inspired campus** (`src/data/maharashtra/campus.ts`, `src/world/hero/Campus.ts`) — an original, fictionalised campus of box geometry at the approximate public map position near Atigre, Kolhapur. No affiliation, no photographs, plans, logos or names of the real school are used; the gate label and the HUD say "Original, fictionalised campus inspired by SGIS — not the school's actual layout".
